...
export default function NominationsView() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: nominations } = await supabase.from("nominations").select("id, driver_id, paid, nomination_entries(class_id, is_preference, order_index)").eq("event_id", eventId);
